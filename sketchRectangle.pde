  ...
  void EffaceRectangle(Rectangle[][] Mur) {
    for (int i=0; i< Mur.length; i++) {
      for (int j=0; j < Mur[i].length; j++) {
        ...
        float BallBottom = bal.y+bal.diametre/2;
        float BallTop = bal.y-bal.diametre/2;
        float BallRight = bal.x+bal.diametre/2;
        float BallLeft = bal.x-bal.diametre/2;
